...
export default function QuizzesPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingQuiz) {
        ...
        setQuizzes(
          quizzes.map((q) => (q.id === editingQuiz.id ? { ...q, ...formData } : q))
        );
      } else {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-3xl font-700 text-ink mb-2">Manage Quizzes</h1>
        <p className="text-inksoft font-body">Create and manage course quizzes</p>
      </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-ink font-body">
            Are you sure you want to delete quiz <strong>{deleteConfirm?.title}</strong>? This action cannot be undone.
          </p>
